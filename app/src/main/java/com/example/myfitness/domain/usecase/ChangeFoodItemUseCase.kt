...
class ChangeFoodItemUseCase @Inject constructor(
    ...
    suspend fun execute(food: FoodModel, day: DayFoodItemModel): DayFoodItemModel {
        ...
        val updatedDay = day.copy(
            ...
            breakfast     = if (food.typeOfMeal == "breakfast") replaceIn(day.breakfast, foodOld, food) else day.breakfast,
            lunch         = if (food.typeOfMeal == "lunch")     replaceIn(day.lunch,     foodOld, food) else day.lunch,
            dinner        = if (food.typeOfMeal == "dinner")    replaceIn(day.dinner,    foodOld, food) else day.dinner,
            snacks        = if (food.typeOfMeal == "snacks")    replaceIn(day.snacks,    foodOld, food) else day.snacks
        )
